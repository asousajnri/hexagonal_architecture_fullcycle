...
export default class ParkedCarDatabaseRepository implements ParkedCarRepositoryPort {
	...
	async save(parkedCar: ParkedCar): Promise<void> {
		await this.db.query("insert into public.parked_car (plate, checkin_date) values  ($1, $2)", [parkedCar.plate, parkedCar.checkinDate]);
	}

	async update(parkedCar: ParkedCar): Promise<void> {
		await this.db.query("update public.parked_car set checkout_date = now() where plate = $1", [parkedCar.plate]);
	}

	async list(): Promise<ParkedCar[]> {
		const parkedCarsData = await this.db.query("select * from public.parked_car where checkout_date is null", []);
	...
	async get(plate: string): Promise<ParkedCar> {
		const parkedCarData = await this.db.one("select * from public.parked_car where plate = $1 and checkout_date is null", [plate]);
		const parkedCar = new ParkedCar(parkedCarData.plate, new Date(parkedCarData.checkin_date));
		return parkedCar;
	}
